...
int main() {
	...
	while (true) {
		...
		if (a == 1) {
			cout << "Unesite b: (velicina tabele ce biti 2^b) ";
			cin >> b;
			...
			if (table != nullptr) {
				...
				cin >> ind;
				student.index = ind;
				if (table->find_key(student) != nullptr) {
					cout << "Student je nadjen! " << endl;
				}
				else {
